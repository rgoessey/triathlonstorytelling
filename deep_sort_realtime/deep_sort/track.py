...
class Track:
    ...
    starttime, stoptime : int
        Framenumber where track started and stopped
    triatlete : Triatlete
        To which triatlete this track is coupled to
    """
    ...
    def update(self, kf, detection, triatl):
        ...
        if (self.triatlete is not None) & (triatl is not None):
            self.triatlete.update(triatl)
        elif triatl is not None:
            self.triatlete=Triatlete(triatl["name"],triatl["prob"],triatl["number"])
